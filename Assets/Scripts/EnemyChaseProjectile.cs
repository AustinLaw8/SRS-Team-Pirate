...
public class EnemyChaseProjectile : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector2 projectileToPlayer = transform.position - Player.MyPlayer.transform.position;
        if (distTrav >= range) {
            Destroy(gameObject);
        }
        float step = speed * Time.deltaTime; // calculate distance to move
        transform.position = Vector3.MoveTowards(transform.position, Player.MyPlayer.transform.position, step);
        distTrav += step;
    }
}
